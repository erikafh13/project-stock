...
st.set_page_config(layout="wide", page_title="Analisis Stock & ABCDE")

# --- SIDEBAR ---
st.sidebar.image("https://i.imgur.com/n0KzG1p.png", use_container_width=True)
st.sidebar.title("Analisis Stock dan ABCDE")

page = st.sidebar.radio(
    "Menu Navigasi:",
    ("Input Data", "Hasil Analisa Stock", "Hasil Analisa ABCDE"), # Diubah
...
if 'abc_analysis_result' not in st.session_state: # Tetap menggunakan nama state lama agar tidak reset
...
if page == "Input Data":
    ...
    # --- [MODIFIKASI] FUNGSI-FUNGSI SPESIFIK ANALISA STOCK DENGAN LOGIKA ABCDE ---
    ...
    def classify_abcde(df_group): # Diubah dari classify_abc menjadi classify_abcde
        ...
        def set_category(x):
            if x <= 70: return 'A - Fast'
            elif x <= 90: return 'B - Middle Fast'
            elif x <= 95: return 'C - Middle Slow'
            else: return 'D - Slow'
        ...
        if total_kuantitas > 0:
            ...
            terjual['Kategori ABCDE'] = terjual['% Kumulatif'].apply(set_category)
        else:
            terjual['% kontribusi'] = 0
            terjual['% Kumulatif'] = 0
            terjual['Kategori ABCDE'] = 'E - Dead Sell'

        tidak_terjual['% kontribusi'] = 0
        tidak_terjual['% Kumulatif'] = 100
        tidak_terjual['Kategori ABCDE'] = 'E - Dead Sell'
        
        df_final = pd.concat([terjual, tidak_terjual], ignore_index=True)
        # Hapus kolom lama jika ada untuk menghindari konflik
        if 'Kategori ABC' in df_final.columns:
            df_final = df_final.drop(columns=['Kategori ABC'])
        return df_final
    ...
    def get_z_score(category, volatility): # Diubah untuk menangani 5 kategori
        cat_letter = str(category)[0]
        base_z = {'A': 1.0, 'B': 1.0, 'C': 0.0, 'D': 0.0, 'E': 0.0}.get(cat_letter, 0.0)
    ...
    def highlight_kategori(val): # Diubah untuk 5 kategori
        warna = {
            'A - Fast': 'background-color: #cce5ff',
            'B - Middle Fast': 'background-color: #d4edda',
            'C - Middle Slow': 'background-color: #fff3cd',
            'D - Slow': 'background-color: #ffd6a5',
            'E - Dead Sell': 'background-color: #f8d7da'
        }
    ...
    def get_status_stock(row): # Diubah untuk kategori E
        if str(row['Kategori ABCDE']).startswith('E'): return 'Overstock Dead Sell' if row['Stock Cabang'] > 2 else 'Balance'
    ...
    def highlight_status_stock(val): # Diubah untuk label baru
        colors = {'Understock': '#fff3cd', 'Balance': '#d4edda', 'Overstock': '#ffd6a5', 'Overstock Dead Sell': '#f5c6cb'}
        return f'background-color: {colors.get(val, "")}'

    def calculate_max_stock(avg_wma, category): # Diubah untuk 5 kategori
        cat_letter = str(category)[0]
        multiplier = {'A': 2.0, 'B': 1.5, 'C': 1.0, 'D': 0.5, 'E': 0.0}
        return avg_wma * multiplier.get(cat_letter, 0)
    ...
    if st.button("Jalankan Analisa Stock"):
        with st.spinner("Melakukan perhitungan analisis stok..."):
            ...
            if penjualan_for_wma.empty:
                ...
                # [MODIFIKASI] Memanggil fungsi classify_abcde
                final_result = full_data.groupby('City', group_keys=False).apply(classify_abcde).reset_index(drop=True)
                
                bulan_columns = [col for col in final_result.columns if isinstance(col, pd.Period)]
                # [MODIFIKASI] Menggunakan Kategori ABCDE untuk perhitungan
                final_result['Safety Stock'] = final_result.apply(lambda row: calculate_safety_stock_from_series(row[bulan_columns].tolist(), row['Kategori ABCDE']), axis=1)
                ...
                final_result['Max Stock'] = final_result.apply(lambda row: calculate_max_stock(row['AVG WMA'], row['Kategori ABCDE']), axis=1)
    ...
    if st.session_state.stock_analysis_result is not None:
        ...
        # [MODIFIKASI] Filter berdasarkan Kategori ABCDE
        abc_options = sorted(final_result_display['Kategori ABCDE'].dropna().unique().astype(str))
        selected_abc = col_h1.multiselect("Kategori ABCDE:", abc_options)
        ...
        with tab1:
            ...
            for city in sorted(final_result_display['City'].unique()):
                with st.expander(f"📍 Lihat Hasil Stok untuk Kota: {city}"):
                    ...
                    if selected_abc: city_df = city_df[city_df['Kategori ABCDE'].isin(selected_abc)]
                    ...
                    st.dataframe(
                        city_df.style.applymap(highlight_kategori, subset=['Kategori ABCDE'])
            ...
            with st.spinner("Membuat tabel pivot gabungan untuk stok..."):
                if final_result_display.empty:
                    ...
                    # [MODIFIKASI] Mengubah kolom Kategori ABC menjadi ABCDE di pivot
                    pivot_cols = ['AVG WMA', 'AVG Mean', 'Penjualan Bln 1', 'Penjualan Bln 2', 'Penjualan Bln 3', 'Kategori ABCDE', 'Min Stock', 'Safety Stock', 'ROP', 'Max Stock', 'Stock Cabang', 'Status Stock', 'Add Stock', 'Suggested PO']
                    ...
                    metric_order = ['AVG WMA', 'AVG Mean', 'Penjualan Bln 1', 'Penjualan Bln 2', 'Penjualan Bln 3', 'Kategori ABCDE', 'Min Stock', 'Safety Stock', 'ROP', 'Max Stock', 'Stock Cabang', 'Status Stock', 'Add Stock', 'Suggested PO']
                    ...
                    # [MODIFIKASI] Logika klasifikasi untuk data gabungan
                    all_sales_for_abc = total_agg.copy()
                    all_sales_for_abc.rename(columns={'All_SO': 'Total Kuantitas'}, inplace=True)
                    all_classified = classify_abcde(all_sales_for_abc) # Menggunakan fungsi yang sudah dimodifikasi
                    all_classified.rename(columns={'Kategori ABCDE': 'All_Kategori ABCDE All'}, inplace=True)
                    ...
                    pivot_result = pd.merge(pivot_result, all_classified[keys + ['All_Kategori ABCDE All']], on=keys, how='left')
                    
                    final_summary_cols = ['All_Stock', 'All_SO', 'All_Suggested_PO', 'All_Kategori ABCDE All', 'All_Restock 1 Bulan']
        ...
        with tab2:
            ...
            if not final_result_display.empty:
                ...
                with col_chart1:
                    st.subheader("Distribusi Kategori ABCDE")
                    abc_counts = final_result_display['Kategori ABCDE'].value_counts()
...
elif page == "Hasil Analisa ABCDE": # Diubah dari ABC
    st.title("📊 Analisis ABCDE Berdasarkan Metrik Penjualan Dinamis")
    tab1_abc, tab2_abc = st.tabs(["Hasil Tabel", "Dashboard"])
    
    with tab1_abc:
        # [MODIFIKASI] Fungsi classify_abc_dynamic diubah menjadi classify_abcde_dynamic
        def classify_abcde_dynamic(df_grouped, metric_col='Metrik_Penjualan'):
            abc_results = []
            if 'City' not in df_grouped.columns:
                if not df_grouped.empty:
                    st.warning("Kolom 'City' tidak ditemukan dalam data untuk klasifikasi ABCDE.")
                return pd.DataFrame()
            for city, city_group in df_grouped.groupby('City'):
                ...
                if total_metric == 0:
                    terjual['% kontribusi'] = 0; terjual['% Kumulatif'] = 0; terjual['Kategori ABCDE'] = 'E - Dead Sell'
                else:
                    terjual['% kontribusi'] = 100 * terjual[metric_col] / total_metric
                    terjual['% Kumulatif'] = terjual['% kontribusi'].cumsum()
                    terjual['Kategori ABCDE'] = terjual['% Kumulatif'].apply(set_category)
                
                tidak_terjual['% kontribusi'] = 0; tidak_terjual['% Kumulatif'] = 100; tidak_terjual['Kategori ABCDE'] = 'E - Dead Sell'
        ...
        # [MODIFIKASI] Fungsi highlight disesuaikan untuk 5 kategori
        def highlight_kategori_abcde(val):
            warna = {
                'A - Fast': 'background-color: #cce5ff',
                'B - Middle Fast': 'background-color: #d4edda',
                'C - Middle Slow': 'background-color: #fff3cd',
        ...
        st.header("Filter Rentang Waktu Analisis ABCDE")
        ...
        if st.button("Jalankan Analisa ABCDE"): # Tombol diubah
            with st.spinner("Melakukan perhitungan analisis ABCDE..."):
                ...
                # [MODIFIKASI] Memanggil fungsi baru
                result = classify_abcde_dynamic(grouped, metric_col='Metrik_Penjualan')
                st.session_state.abc_analysis_result = result.copy()
                st.success("Analisis ABCDE berhasil dijalankan!")
        ...
        if st.session_state.abc_analysis_result is not None:
            ...
            st.header("Hasil Analisis ABCDE per Kota")
            for city in sorted(result_display['City'].unique()):
                with st.expander(f"🏙️ Lihat Hasil ABCDE untuk Kota: {city}"):
                    city_df = result_display[result_display['City'] == city]
                    # [MODIFIKASI] Mengganti nama kolom Kategori ABC menjadi ABCDE
                    display_cols_order = ['No. Barang', 'BRAND Barang', 'Nama Barang', 'Kategori Barang', 'Metrik_Penjualan', 'Kategori ABCDE', '% kontribusi', '% Kumulatif']
                    df_city_display = city_df[display_cols_order]
                    # [MODIFIKASI] Memanggil fungsi highlight baru
                    st.dataframe(df_city_display.style.format({'Metrik_Penjualan': '{:.2f}', '% kontribusi': '{:.2f}%', '% Kumulatif': '{:.2f}%'}).apply(lambda x: x.map(highlight_kategori_abcde), subset=['Kategori ABCDE']), use_container_width=True)
            
            st.header("📊 Tabel Gabungan Seluruh Kota (ABCDE)")
            with st.spinner("Membuat tabel pivot gabungan untuk ABCDE..."):
                keys = ['No. Barang', 'Kategori Barang', 'BRAND Barang', 'Nama Barang']
                # [MODIFIKASI] Menggunakan Kategori ABCDE di pivot
                pivot_abc = result_display.pivot_table(index=keys, columns='City', values=['Metrik_Penjualan', 'Kategori ABCDE'], aggfunc={'Metrik_Penjualan': 'sum', 'Kategori ABCDE': 'first'})
                ...
                total_abc['City'] = 'All'
                # [MODIFIKASI] Memanggil fungsi baru untuk data gabungan
                total_abc_classified = classify_abcde_dynamic(total_abc.rename(columns={'Total_Metrik': 'Metrik_Penjualan'}), metric_col='Metrik_Penjualan')
                total_abc_classified.rename(columns={'Kategori ABCDE': 'All_Kategori_ABCDE', '% kontribusi': 'All_%_Kontribusi'}, inplace=True)
                
                pivot_abc_final = pd.merge(pivot_abc, total_abc_classified[keys + ['All_Kategori_ABCDE', 'All_%_Kontribusi']], on=keys, how='left')
                st.dataframe(pivot_abc_final, use_container_width=True)
                
            st.header("💾 Unduh Hasil Analisis ABCDE")
            output_abc = BytesIO()
            ...
            st.download_button("📥 Unduh Hasil Analisis ABCDE (Excel)",data=output_abc.getvalue(),file_name=f"Hasil_Analisis_ABCDE_{start_date}_sd_{end_date}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    with tab2_abc:
        st.header("📈 Dashboard Analisis ABCDE")
        if 'abc_analysis_result' in st.session_state and st.session_state.abc_analysis_result is not None:
            ...
            if not result_display_dash.empty:
                # [MODIFIKASI] Grouping berdasarkan Kategori ABCDE
                abc_summary = result_display_dash.groupby('Kategori ABCDE')['Metrik_Penjualan'].agg(['count', 'sum'])
                ...
                st.markdown("---")
                # [MODIFIKASI] Menampilkan metrik untuk 5 kategori
                col1, col2, col3, col4, col5 = st.columns(5)
                if 'A - Fast' in abc_summary.index:
                    col1.metric("A - Fast", f"{abc_summary.loc['A - Fast', 'count']} SKU", f"{abc_summary.loc['A - Fast', 'sum_perc']:.1f}% Penjualan")
                if 'B - Middle Fast' in abc_summary.index:
                    col2.metric("B - Middle Fast", f"{abc_summary.loc['B - Middle Fast', 'count']} SKU", f"{abc_summary.loc['B - Middle Fast', 'sum_perc']:.1f}% Penjualan")
                if 'C - Middle Slow' in abc_summary.index:
                    col3.metric("C - Middle Slow", f"{abc_summary.loc['C - Middle Slow', 'count']} SKU", f"{abc_summary.loc['C - Middle Slow', 'sum_perc']:.1f}% Penjualan")
                if 'D - Slow' in abc_summary.index:
                    col4.metric("D - Slow", f"{abc_summary.loc['D - Slow', 'count']} SKU", f"{abc_summary.loc['D - Slow', 'sum_perc']:.1f}% Penjualan")
                if 'E - Dead Sell' in abc_summary.index:
                    col5.metric("E - Dead Sell", f"{abc_summary.loc['E - Dead Sell', 'count']} SKU", "Tidak Terjual")
